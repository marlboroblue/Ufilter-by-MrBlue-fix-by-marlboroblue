    ...
    public class UFilter : CovalencePlugin
    {
        ...
        private new void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["WordAdded"] = "Word/phrase '{0}' was added to the profanity list",
                ["WordListed"] = "Word/phrase '{0}' is already in the profanity list",
                ["WordNotListed"] = "Word/phrase '{0}' is not in the profanity list",
                ["WordRemoved"] = "Word/phrase '{0}' was removed from the profanity list",
            }, this);
        }
        ...
        private class StoredData
        {
            ...
            public readonly HashSet<string> ProfanityPhrases = new HashSet<string>();
        }
        ...
        private string[] Profanities(string text)
        {
            ...
            // Check single words
            var singleWords = Regex.Split(text, @"\W")
                .Where(w => storedData.Profanities.Contains(w.ToLower()) && !config.AllowedProfanity.Contains(w.ToLower()))
                .ToArray();
            
            // Check phrases
            var phrases = storedData.ProfanityPhrases
                .Where(phrase => lowerText.Contains(phrase.ToLower()) && !config.AllowedProfanity.Contains(phrase.ToLower()))
                .ToArray();
            
            return singleWords.Concat(phrases).ToArray();
        }
        ...
        private void ProcessName(IPlayer player)
        {
            ...
            {
                player.Rename("Unnamed" + new System.Random().Next(1000, 9999));
            }
        }
        ...
        private string[] GetProfanityPhrases() => storedData?.ProfanityPhrases.ToArray();
        ...
        private bool AddProfanity(string profanity)
        {
            ...
            if (profanity.Contains(" "))
            {
                if (storedData.ProfanityPhrases.Contains(profanity))
                    return false;
                    
                storedData.ProfanityPhrases.Add(profanity);
                added = true;
            }
            else
            {
                if (storedData.Profanities.Contains(profanity))
                    return false;
        ...
        private bool RemoveProfanity(string profanity)
        {
            ...
            if (profanity.Contains(" "))
            {
                removed = storedData.ProfanityPhrases.Remove(profanity);
            }
            else
            {
                removed = storedData.Profanities.Remove(profanity);
            }
        ...
        private void FilterCommand(IPlayer player, string command, string[] args)
        {
            ...
            switch (args[0].ToLower())
            {
                ...
                case "list":
                    string words = string.Join(", ", storedData.Profanities);
                    string phrases = string.Join(", ", storedData.ProfanityPhrases);
                    string message = "Words: " + (string.IsNullOrEmpty(words) ? "None" : words);
                    message += "\nPhrases: " + (string.IsNullOrEmpty(phrases) ? "None" : phrases);
